...
class Preferences internal constructor(
    ...
    private val connection: EntityManager,
) {
        ...
        connection
            .createNativeQuery(
                "UPDATE tasks " +
                    ...
                    "WHERE channel_id = :channelId AND task_name='ask_question'",
    ...
    fun autoAskEveryXMinutes(minutes: Int) {
        connection
            .createNativeQuery(
                "UPDATE tasks SET frequency = :frequency " +
                    "WHERE channel_id = :channelId AND task_name='ask_question'",
            ).setParameter("channelId", group.id)
    ...
    fun enableFactSharing(status: Boolean) {
        connection
            .createNativeQuery(
                "UPDATE tasks SET is_active = :isActive " +
                    "WHERE channel_id = :channelId AND task_name='share_fact'",
        ...
        connection
            .createNativeQuery(
                "UPDATE tasks " +
                    ...
                    "WHERE channel_id=? AND task_name='share_fact'",
            ).setParameter("frequency", minutes)
            .setParameter("channelId", group.id)
    }
}
